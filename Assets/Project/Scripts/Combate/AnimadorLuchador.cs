...
public class AnimadorLuchador : MonoBehaviour
{
    ...
    public IEnumerator ReproducirVictoria(string estadoVictoria)
    {
        ...
        if (!animator.HasState(0, Animator.StringToHash(estadoVictoria)))
        {
            bloquearPosicion = false;
            Debug.LogWarning("No existe el estado de victoria en el Animator: " + estadoVictoria);
            yield break;
        }

        ReproducirEstado(estadoVictoria);

        float duracion = ObtenerDuracionClip(estadoVictoria);
        if (duracion <= 0f)
        {
            Debug.LogWarning("No se encontro la animacion de victoria: " + estadoVictoria);
            animator.Play(estadoVictoria, 0, 1f);
            animator.Update(0f);
            animator.speed = 0f;
            yield break;
        }
